...
const NewsDetail: React.FC = () => {
  ...
  if (!newsItem) {
    return (
      <section className="py-20 bg-gray-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <button onClick={() => navigate(-1)} className="mb-6 text-orange-600 font-medium hover:text-orange-700">
            Kembali
          </button>
          <div className="text-center text-gray-600">Berita tidak ditemukan</div>
        </div>
      </section>
    );
  }
  ...
  const renderDescription = (description: string | null | undefined) => {
    ...
    return (
      <>
        ...
        {conclusion && <p className="mt-4 text-gray-700 font-medium leading-relaxed">{conclusion}</p>}
      </>
    );
  };
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <article className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="relative h-96 overflow-hidden bg-slate-100">
            <img
              src={newsItem.thumbnail_url || "https://placehold.co/600x400?text=No+Image"}
              alt={newsItem.title}
              className="w-full h-full object-cover"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
            <div className="absolute top-4 left-4 bg-white/85 px-3 py-1 rounded-full text-sm font-medium text-gray-700">
              <Calendar className="h-4 w-4 inline mr-1" />
              {formatDate(newsItem.published_date)}
            </div>
            <div className="absolute top-4 right-4 bg-white/85 px-3 py-1 rounded-full text-sm font-medium text-gray-700">
          ...
          <div className="p-6">
            ...
            <div className="text-gray-600 leading-relaxed mb-6">{renderDescription(newsItem.content)}</div>
          </div>
        </article>
      </div>
    </section>
